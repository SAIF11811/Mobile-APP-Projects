...
class _ListItemSelectorState extends State<ListItemSelector> {
  ...
  void initState() {
    ...
    // If no item is selected, automatically select the first one
    if (widget.categories.isNotEmpty &&
        !widget.categories.any((cat) => cat.isSelected)) {
      widget.categories[0].isSelected = true;
      widget.onItemPressed(0); // Notify parent
    }
  }
    ...
    return Tooltip(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: item.isSelected ? const Color(0xFFf16b26) : const Color(0xFFE5E6E8),
        ...
        child: IconButton(
          ...
          icon: FaIcon(
            item.icon,
            color: item.isSelected ? Colors.white : const Color(0xFFA6A3A0),
          ),
          onPressed: () {
