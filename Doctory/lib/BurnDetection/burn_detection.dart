...
class BurnDetectionScreen extends StatefulWidget {
  const BurnDetectionScreen({super.key});

  @override
  State<BurnDetectionScreen> createState() => _BurnDetectionScreenState();
}

class _BurnDetectionScreenState extends State<BurnDetectionScreen> {
  Interpreter? _interpreter;
  final picker = ImagePicker();
  File? _image;
  String _result = 'No Photo Yet';
  ...
  Future<void> loadModel() async {
    try {
      _interpreter = await Interpreter.fromAsset('assets/mobilenet_burn_model.tflite');
      _interpreter!.allocateTensors();
    } catch (e) {
      setState(() => _result = '❌ Failed to load model.\n$e');
    }
  }
  ...
  Future<void> predict(File imageFile) async {
    ...
    try {
      ...
      double sumExp = logits.map((e) => exp(e)).reduce((a, b) => a + b);
      List<double> probabilities = logits.map((e) => exp(e) / sumExp).toList();
      ...
      String resultText = 'Result: ${classLabels[predictedIndex]}';

      setState(() => _result = resultText);
    } catch (e) {
      setState(() => _result = 'Prediction failed.$e');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarOfScreen(context, text: 'Burn Detection'),
      body: Center(
